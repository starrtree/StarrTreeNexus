...
export function PlanningAgentDockV2({
  open,
  onClose,
  initialPrompt,
  autoRunNonce,
  ...
  useEffect(() => {
    if (!open || !initialPrompt || !autoRunNonce) return;
    askPlanner(initialPrompt);
  ...
  function applyAction(action: PlannerAction, index: number) {
    ...
    if (action.type === "create_idea") {
      const idea = {
        ...
        impact: score(action.impact, 7),
        cash: 5,
        effort: 5,
        brand: 7,
        urgency: score(action.urgencyScore, 6),
        status: "new" as const,
        createdAt: Date.now(),
      };
          ...
          >
            ...
            {loading && (
              <div className="mt-4 rounded-2xl border border-sky-400/25 bg-sky-400/10 p-3 text-sm text-sky-100">
                <Loader2 size={14} className="mr-2 inline animate-spin" /> Planning Agent is reading StarrBoard and preparing a response...
              </div>
            )}
              ...
              <div className="mt-4 space-y-4">
                <section className="rounded-2xl border border-amber-300/25 bg-amber-400/10 p-4">
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <h3 className="font-hud text-[11px] font-bold uppercase tracking-widest text-amber-100">Answer</h3>
                    <span className="rounded-full border border-violet-400/25 bg-black/25 px-2 py-0.5 font-hud text-[8px] uppercase tracking-widest text-violet-100/60">
                      {confidence}% confidence
                    </span>
                  </div>
                  <p className="text-sm leading-relaxed text-amber-50/90">{result.summary}</p>
                </section>

                {result.activityDigest?.length > 0 && (
                  <Panel title="Activity Digest" icon={<Sparkles size={13} />}>
                    {result.activityDigest.map((item) => <Bullet key={item}>{item}</Bullet>)}
                  </Panel>
                )}

                {result.recommendedPriorities?.length > 0 && (
                  <Panel title="Recommended Priorities" icon={<BrainCircuit size={13} />}>
                    <div className="space-y-2">
                      {result.recommendedPriorities.map((priority) => (
                        <div key={`${priority.title}-${priority.why}`} className="rounded-xl border border-violet-400/15 bg-black/20 p-3">
                          <div className="flex items-center justify-between gap-2">
                            <span className="text-sm font-semibold text-amber-50">{priority.title}</span>
                            <span className={cn("rounded-full border px-2 py-0.5 font-hud text-[8px] uppercase tracking-widest", priorityTone(priority.urgency))}>
                              {priority.urgency}
                            </span>
                          </div>
                ...
                {result.suggestedActions?.filter((a) => a.type !== "none").length > 0 && (
                  <Panel title="Suggested Actions" icon={<CheckCircle2 size={13} />}>
                    <div className="space-y-2">
                      {result.suggestedActions.filter((a) => a.type !== "none").map((action, index) => (
                        <div key={`${action.type}-${action.label}-${index}`} className="rounded-xl border border-violet-400/15 bg-black/20 p-3">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <div className="text-sm font-semibold text-amber-50">{action.label}</div>
                              <p className="mt-1 text-xs leading-relaxed text-violet-100/65">{action.rationale}</p>
                            </div>
                            <button
                              onClick={() => applyAction(action, index)}
                              disabled={appliedActions[index]}
                              className="shrink-0 rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-3 py-1.5 font-hud text-[9px] uppercase tracking-widest text-emerald-100 transition hover:bg-emerald-400/20 disabled:opacity-50"
                            >
                ...
                {result.questions?.length > 0 && (
                  <Panel title="Questions" icon={<AlertTriangle size={13} />}>
...
function priorityTone(priority: Priority) {
  if (priority === "urgent") return "border-red-300/35 bg-red-400/10 text-red-100";
...
function Panel({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <section className="rounded-2xl border border-violet-400/20 bg-white/[0.03] p-4">
      <div className="mb-2 flex items-center gap-2 font-hud text-[10px] font-bold uppercase tracking-widest text-violet-200/70">
        {icon}
        {title}
      </div>
      {children}
...
function Bullet({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-1.5 flex gap-2 text-sm leading-relaxed text-violet-100/70">
